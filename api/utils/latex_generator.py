...
def generate_pdf_response_from_json(json_data):
    try:
        html = render_to_string("api/templates/pdf_generators/pdf_view.html", {
            "test_name": json_data["test_name"],
            "subjects": json_data["subjects"],
            "subject_name": json_data["subjects"][0]["subject_name"],
            "logo_url": os.path.join(settings.STATIC_ROOT, "logo.png"),  # or static URL if using CDN
        })
